...
int PositionIs_Fct(auto handle)
{
	unsigned int PositionIsError;
	int PositionIs;
	auto GetPositionIs = VCS_GetPositionIs(handle, 1, &PositionIs, &PositionIsError);
	return PositionIs;
}

//move cylinder to position x
void SetPosition(auto handle, int position)
{
	unsigned int pErrorPosMust (0);
	auto setPosMust = VCS_SetPositionMust(handle, 1, position, &pErrorPosMust);
	std::cout << setPosMust << " " << pErrorPosMust << std::endl;	
}

//move cylinder forward. Move by 1000 at a time, works well (smaller steps won't be recognized)
void MoveXForward(auto handle, int ForwardMovement)
{
	int PositionIs = PositionIs_Fct(handle);
	int NewPosition = PositionIs+ForwardMovement;
	SetPosition(handle, NewPosition);
}

//move cylinder backward. Move by 1000 at a time, works well (smaller steps won't be recognized)
void MoveXBackward(auto handle, int BackwardMovement)
{
	int PositionIs = PositionIs_Fct(handle);
...
void SetDigitalOutputs(auto handle, unsigned int ActivationKey)
{	
	unsigned int SetOutputError;
	auto SetOutputs = VCS_SetAllDigitalOutputs(handle, 1, ActivationKey, &SetOutputError);
	std::cout << SetOutputs << " " << SetOutputError << std::endl;	
}

void PrintPosition(auto handle)
{
	unsigned int PositionIsError;
	int PositionIs;
	auto GetPositionIs = VCS_GetPositionIs(handle, 1, &PositionIs, &PositionIsError);
...
bool LimitReachedFunction(auto handle)
{
	//unsigned short DigitalInput6;
	unsigned int ErrorInput;
	unsigned short DigitalInput;
	auto GetDigitalInput=VCS_GetAllDigitalInputs(handle, 1, &DigitalInput, &ErrorInput);
	//DigitalInput6 = DigitalInput & 0x40;
	std::cout <<"Digital Input Error State:" << GetDigitalInput << " " << ErrorInput << std::endl;
	std::cout <<"Digital Input:" << DigitalInput << std::endl;

	return (!DigitalInput);
}

//go to minimal extension position
void GoToMin(auto handle)
{
	SetPosition(handle, -5000);
	PrintPosition(handle);
	std::cout << "Minimal extension position reached! No further movements in this direction!!" << std::endl;
}

//go to maximal extension position
void GoToMax(auto handle)
{
	SetPosition(handle, 55000);
	PrintPosition(handle);
	std::cout << "Maximal extension position reached! No further movements in this direction!!" << std::endl;
}

//go to middle extension position
void GoToMid(auto handle)
{
	SetPosition(handle, 25000);
	PrintPosition(handle);
	std::cout << "Middle extension position reached." << std::endl;
}

// Activate position mode
void PositionMode(auto handle)
{
	unsigned int pErrorActPosMode;
	auto actPosMode = VCS_ActivatePositionMode(handle, 1, &pErrorActPosMode);
	std::cout << actPosMode << " " << pErrorActPosMode << std::endl;

	bool LimitReached = LimitReachedFunction(handle);
	long double t(0);
	int dt (100);
	while (t < 100000 && !LimitReached)
	{
		LimitReached = LimitReachedFunction(handle);
...
int main(int argc, char** argv)
{
	...
	auto open = VCS_OpenDevice(deviceNameCharPtr, protocolStackNameCharPtr, interfaceNameCharPtr, portNameCharPtr, &pErrorCodeOpen);
	std::cout << open << " " << pErrorCodeOpen << std::endl;

	// Set protocol stack settings
	unsigned int pErrorCodeProtocol;
	auto protocol = VCS_SetProtocolStackSettings(open, 1e6, 500, &pErrorCodeProtocol);
	std::cout << protocol << " " << pErrorCodeProtocol << std::endl;

	// Disable state
	unsigned int pErrorDisableState;
	VCS_SetDisableState(open, 1, &pErrorDisableState);

        // Set motor type
        unsigned int pErrorCodeMotor;
        auto motorType = VCS_SetMotorType(open, 1, 10, &pErrorCodeMotor);
        std::cout << motorType << " " << pErrorCodeMotor << std::endl;

        // Set motor parameters !! units unsure (mA, mA, s/10)
        unsigned int pErrorEc;
        auto ecMotor = VCS_SetEcMotorParameter(open, 1, 6210, 10000, 310, 1, &pErrorEc);
        std::cout << ecMotor << " " << pErrorEc << std::endl;
	
	// Set sensor type
	unsigned int pErrorSensorType;
        auto sensorType = VCS_SetSensorType(open, 1, 1, &pErrorSensorType);
        std::cout << sensorType << " " << pErrorSensorType << std::endl;

	// Set sensor parameter
	unsigned int pErrorSensorParam;
	auto sensorParam = VCS_SetIncEncoderParameter(open, 1, 500, 0, &pErrorSensorParam);
	std::cout << sensorParam << " " << pErrorSensorParam << std::endl;

        // Set position regulator gain
        unsigned int pErrorPosRegGain;
        auto positionRegGain = VCS_SetPositionRegulatorGain(open, 1, 798, 3151, 1078, &pErrorPosRegGain);
        std::cout << positionRegGain << " " << pErrorPosRegGain << std::endl;

        // Set sensor parameter
        unsigned int pErrorVelRegGain;
        auto velocityRegGain = VCS_SetVelocityRegulatorGain(open, 1, 2874, 532, &pErrorVelRegGain);
        std::cout << velocityRegGain << " " << pErrorVelRegGain << std::endl;

        // Set sensor parameter
        unsigned int pErrorCurrentRegGain;
        auto currentRegGain = VCS_SetCurrentRegulatorGain(open, 1, 335, 60, &pErrorCurrentRegGain);
        std::cout << currentRegGain << " " << pErrorCurrentRegGain << std::endl;

	// Set enable state
	unsigned int pErrorState;
	auto state = VCS_SetEnableState(open, 1, &pErrorState);
	...
	/*
        ...
        auto profilePosMode = VCS_ActivateProfilePositionMode(open, 1, &pErrorProfilePosMode);
        std::cout << profilePosMode << " " << pErrorProfilePosMode << std::endl;

        // Set sensor parameter
        unsigned int pErrorSetPosProfile;
        auto posProfile = VCS_SetPositionProfile(open, 1, 1900, 9300, 9300, &pErrorSetPosProfile);
	...
	bool immediately = false;
        auto moveToPos1 = VCS_MoveToPosition(open, 1, -24000, absoluteMovement, immediately, &pErrorMoveToPos);
   	auto moveToPos2 = VCS_MoveToPosition(open, 1, 24000, absoluteMovement, immediately, &pErrorMoveToPos);

        std::cout << moveToPos1 << " " << moveToPos2 << " " << pErrorMoveToPos << std::endl;
...
/*	unsigned int pErrorIntPosMode;
	auto actIntPos = VCS_ActivateInterpolatedPositionMode(open, 1, &pErrorIntPosMode);
	std::cout << actIntPos << " " << pErrorIntPosMode << std::endl;

	// auto setIpmBuffer = VCS_SetIpmBufferParameter(open, 1, -100, 100, &pErrorIntPosMode);
	auto clearIpmBuffer = VCS_ClearIpmBuffer(open, 1, &pErrorIntPosMode);
	...
	{	
		...
		auto addPvt = VCS_AddPvtValueToIpmBuffer(open, 1, position, velocity, t, &pErrorAddPvt);
		std::cout << addPvt << " " << pErrorAddPvt << std::endl;		
	}
	
	auto startIpmTraj = VCS_StartIpmTrajectory(open, 1, &pErrorAddPvt);
	std::cout << startIpmTraj << " " << pErrorAddPvt << std::endl; */
}
